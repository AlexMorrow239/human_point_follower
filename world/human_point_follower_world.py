...
class HumanTracker:
    ...
    def track_human(self, current_time):
        """Perform tracking control using head and base movements"""
        ...
        error_y = center_y - self.target_y
        ...
        # Check if head is near limits for pan
        head_near_pan_limit = False
        if self.current_head_pan is not None:
            margin = 0.2  # radians (~11 degrees) from limit
            if (self.current_head_pan < self.head_pan_min + margin or 
                self.current_head_pan > self.head_pan_max - margin):
                head_near_pan_limit = True
        
        # Decide control strategy
        if head_near_pan_limit and abs(error_x) > self.deadband_x:
            # Use base rotation when head reaches limits
            rospy.loginfo_throttle(1.0, f"Head at pan limit ({math.degrees(self.current_head_pan):.1f}°), using base rotation")
            
            # Compute base angular velocity
            base_angular_vel = -self.base_pid.compute(error_x, current_time)
            ...
            # Gradually center head while base is rotating
            if self.current_head_pan is not None:
                target_head_pan = 0.0  # Center position
                head_centering_rate = 0.5  # rad/s
                
                pan_error = target_head_pan - self.current_head_pan
                pan_command = self.current_head_pan + min(max(pan_error * 0.1, -head_centering_rate * 0.05), 
                                                          head_centering_rate * 0.05)
                
                pan_msg = Float64()
                pan_msg.data = pan_command
                self.head_pan_pub.publish(pan_msg)
        else:
            # Use head tracking when within limits
            # Stop base if it was rotating
            if head_near_pan_limit:
                self.stop_base()
            
            # Compute head velocities using PID
            pan_velocity = -self.pan_pid.compute(error_x, current_time)  # Negative for correct direction
            tilt_velocity = self.tilt_pid.compute(error_y, current_time)
            
            # Update head position based on velocity
            if self.current_head_pan is not None and self.current_head_tilt is not None:
                # Integrate velocity to get position
                dt = 0.05  # 20Hz rate
                new_pan = self.current_head_pan + pan_velocity * dt
                new_tilt = self.current_head_tilt + tilt_velocity * dt
                ...
                # Send commands
                pan_msg = Float64()
                pan_msg.data = new_pan
                tilt_msg = Float64()
                tilt_msg.data = new_tilt
        ...
        rospy.loginfo_throttle(
            1.0, 
            f"Tracking - Error: ({error_x:.0f}, {error_y:.0f}) | "
            f"Center: ({center_x:.0f}, {center_y:.0f}) | "
            f"Head: (Pan: {math.degrees(self.current_head_pan or 0):.1f}°, "
            f"Tilt: {math.degrees(self.current_head_tilt or 0):.1f}°)"
        )
    ...
    def run(self):
        ...
        if self.state == "LOST_HUMAN":
            rospy.loginfo("=" * 50)
            rospy.loginfo("TRACKING TERMINATED: Human lost")
            rospy.loginfo("=" * 50)
        elif self.state == "SCAN_COMPLETE":
            rospy.loginfo("=" * 50)
